...
| File | Responsibility |
|------|----------------|
| `custom_components/ajax_cobranded/api/hts/__init__.py` | Package exports |
| `custom_components/ajax_cobranded/api/hts/protocol.py` | Frame encode/decode: STX/ETX, escape, CRC-16, pad |
| `custom_components/ajax_cobranded/api/hts/crypto.py` | AES-128-CBC encrypt/decrypt |
| `custom_components/ajax_cobranded/api/hts/messages.py` | Message build/parse, TLV params |
| `custom_components/ajax_cobranded/api/hts/auth.py` | Auth handshake orchestration |
| `custom_components/ajax_cobranded/api/hts/client.py` | Async TCP+TLS client with ACK/ping |
| `custom_components/ajax_cobranded/api/hts/hub_state.py` | Parse updates → HubNetworkState |
...
| `custom_components/ajax_cobranded/coordinator.py` | Add HTS integration |
| `custom_components/ajax_cobranded/binary_sensor.py` | Add ethernet/power sensors |
| `custom_components/ajax_cobranded/sensor.py` | Add network sensors |
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/__init__.py`
- Create: `custom_components/ajax_cobranded/api/hts/protocol.py`
...
class TestCrc16:
    def test_empty(self):
        from custom_components.ajax_cobranded.api.hts.protocol import crc16
...
Create `custom_components/ajax_cobranded/api/hts/__init__.py` (empty).
...
class TestEscape:
    def test_no_special_bytes(self):
        from custom_components.ajax_cobranded.api.hts.protocol import escape, unescape
...
class TestPad16:
    def test_already_aligned(self):
        from custom_components.ajax_cobranded.api.hts.protocol import pad16
        data = b"\x00" * 16
...
class TestEncodeDecodeFrame:
    def test_roundtrip(self):
        from custom_components.ajax_cobranded.api.hts.protocol import decode_frame, encode_frame
    ...
    def test_bad_crc_raises(self):
        from custom_components.ajax_cobranded.api.hts.protocol import encode_frame, decode_frame
    ...
    def test_missing_stx_raises(self):
        from custom_components.ajax_cobranded.api.hts.protocol import decode_frame
...
```bash
git add custom_components/ajax_cobranded/api/hts/__init__.py \
        custom_components/ajax_cobranded/api/hts/protocol.py \
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/crypto.py`
...
class TestEncryptDecrypt:
    ...
    def test_input_must_be_block_aligned(self):
        from custom_components.ajax_cobranded.api.hts.crypto import encrypt
        import pytest
...
```bash
git add custom_components/ajax_cobranded/api/hts/crypto.py tests/unit/hts/test_crypto.py
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/messages.py`
...
class TestTlvParams:
    def test_encode_single_param(self):
        from custom_components.ajax_cobranded.api.hts.messages import tlv_encode
    ...
    def test_decode_roundtrip(self):
        from custom_components.ajax_cobranded.api.hts.messages import tlv_decode, tlv_encode
    ...
    def test_decode_empty(self):
        from custom_components.ajax_cobranded.api.hts.messages import tlv_decode
        assert tlv_decode(b"") == []
...
class TestMessageHeader:
    ...
    def test_parse_too_short_raises(self):
        from custom_components.ajax_cobranded.api.hts.messages import parse_message
...
```bash
git add custom_components/ajax_cobranded/api/hts/messages.py tests/unit/hts/test_messages.py
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/hub_state.py`
...
class TestParseHubUpdate:
    def test_parse_active_channels(self):
        from custom_components.ajax_cobranded.api.hts.hub_state import parse_hub_params
    ...
    def test_merge_with_existing(self):
        from custom_components.ajax_cobranded.api.hts.hub_state import HubNetworkState, parse_hub_params
...
```bash
git add custom_components/ajax_cobranded/api/hts/hub_state.py tests/unit/hts/test_hub_state.py
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/auth.py`
...
class TestSolveChallenge:
    def test_basic(self):
        from custom_components.ajax_cobranded.api.hts.auth import solve_challenge
        a, b = 0x10, 0x20
...
class TestBuildConnectRequest:
    def test_contains_password_hash(self):
        from custom_components.ajax_cobranded.api.hts.auth import build_connect_request
        from custom_components.ajax_cobranded.api.hts.messages import tlv_decode
...
class TestParseConnectedResponse:
    def test_parse_hubs(self):
        from custom_components.ajax_cobranded.api.hts.auth import parse_connected_response
        from custom_components.ajax_cobranded.api.hts.messages import tlv_encode
...
```bash
git add custom_components/ajax_cobranded/api/hts/auth.py tests/unit/hts/test_auth.py
...
**Files:**
- Create: `custom_components/ajax_cobranded/api/hts/client.py`
...
```bash
git add custom_components/ajax_cobranded/api/hts/client.py tests/unit/hts/test_client.py
...
```bash
docker run --rm -v $(pwd):/app -w /app \
  -e AJAX_EMAIL='AJAX_EMAIL_REDACTED' \
  -e AJAX_PASSWORD='AJAX_PASSWORD_REDACTED' \
  ajax-cobranded-dev python scripts/test_hts_connection.py
```
...
**Files:**
- Modify: `custom_components/ajax_cobranded/coordinator.py`
- Modify: `custom_components/ajax_cobranded/const.py`
...
In `custom_components/ajax_cobranded/coordinator.py`, add imports and modify `__init__` and `_async_update_data` to start the HTS client alongside gRPC:
...
```bash
git add custom_components/ajax_cobranded/coordinator.py custom_components/ajax_cobranded/const.py
...
- Modify: `custom_components/ajax_cobranded/sensor.py`
